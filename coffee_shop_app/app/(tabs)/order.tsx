...
import { router } from 'expo-router'
  ...
  const Order = () => {
  const {cartItems,setQuantity, emptyCart} = useCart();
  ...
  return (
    ...
    >
      <PageHeader title={"Order"} showHeaderRight={false} bgcolor="#F9F9F9" />
        <View
          className='h-full flex-col justify-between pt-5'
        >
          <View
            className='h-[75%]'
          >
            <ProductList products={products} quantities={cartItems} setQuantities={setQuantity} totalPrice={totalPrice} />
          </View>
          <View
          ...
          <TouchableOpacity 
                className={`${totalPrice=== 0 ? 'bg-[#EDEDED]' : 'bg-app_orange_color' }  2-full rounded-2xl items-center justify-center mt-6 mb-3 py-3`}
